(function() {
  ...
  loadMore.prototype = {
    ...
    checkOptions: function() {
      if (
        !this.options.hasOwnProperty('ele') ||
        !document.querySelector(this.options.ele)
      ) {
        throw new Error(
          'ele必须指定为querySelector可获取的dom元素，用于存放loadmore组件',
        );
      }
      if (
      ...
      if (
        !this.options.hasOwnProperty('totalNum') ||
        !(typeof this.options.totalNum === 'number')
      ) {
        throw new Error('分页器的总页数必须指定,且为Number类型');
      }
      if (
        !this.options.hasOwnProperty('clickFn') ||
        !(typeof this.options.clickFn === 'function')
      ) {
