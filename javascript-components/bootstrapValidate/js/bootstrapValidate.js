(function(root, factory, plug) {
  ...
  function($, plug) {
    ...
    $.fn.valid = function() {
      var $fileds = $(this)
        .find('input')
        .not('[type=button],[type=reset],[type=submit],[type=hidden]');
    ...
    $.fn[plug] = function(options) {
      ...
      var $fileds = this.find('input').not(
        '[type=button],[type=reset],[type=submit],[type=hidden]',
      );
        ...
        $.each(__RULES, function(rule, validator) {
          if ($current.data('bv-' + rule)) {
            ...
            if (!result) {
              ...
              $current.after(
                '<p>' + $current.data('bv-' + rule + '-message') + '</p>',
              );
      ...
      $(this.el).on('submit', function(event) {
        ...
        var $fileds = $(this)
          .find('input')
          .not(
            '[type=button],[type=reset],[type=submit],[type=hidden],[type=radio]',
          );
        for (var i = 0; i < $fileds.length; i++) {
          var $item = $($fileds[i]);
          ...
          $.each(__RULES, function(rule, validator) {
            if ($item.data('bv-' + rule)) {
              ...
              if (!result) {
                ...
                //  console.log(rule+'验证失败','原因是'+$current.data('bv-'+rule+'-message'))
                $item.data('bv-' + rule + '-message') &&
                  $item.after(
                    '<p>' + $item.data('bv-' + rule + '-message') + '</p>',
                  );
                $item.addClass('error');
                return false;
              }
