...
日期： [2024 年 12 月 2 日](https://www.ruanyifeng.com/blog/2024/12/)
...
下面就是开发步骤。为了保证大家跟得上，我写得比较详细，看起来有点长，但是实际操作是很快的，熟练的话，10 分钟就能完成。
...
标题内容设为"AI 文言文生成器"，字号改成 24，字重为粗体。
...
删除 Markdown 组件的已有内容，改成"###### 生成结果"。高度和宽度改为填充容器，圆角设为 10，内边距设为 20，边框设为灰色（#CBD5E1）。
...
事件类型设为"点击时"，执行动作设为"调用 Workflow"，workflow 设为已经创建好的"classicalchinesecreator"，并将工作流的入参`content`设为文本输入框的值 {{ Textarea1.value }}。
...
扣子邀请我，参加**12 月 19 日在上海举行**的"[扣子开发者日](https://mp.weixin.qq.com/s/adDVoDSIxtAMH-havVpvPA)"，也欢迎大家参加。

届时，字节的 AI 工程师会跟大家面对面交流，如何开发和推广 AI 应用。这是很好的学习交流、获取资源的机会，[点击这里](https://mp.weixin.qq.com/s/adDVoDSIxtAMH-havVpvPA)了解详情。

扫描下方海报二 ​ 维码，或者[点击链接](https://www.i-hudong.com/www/FORCE?page=42)，**现在就可以线下报名参加，或者线上预约直播**。
