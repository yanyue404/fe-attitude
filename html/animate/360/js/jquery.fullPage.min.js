...
(function(d, k) {
  'function' === typeof define && define.amd
    ...
    ? (module.exports = k(require('jquery'), d, d.document, d.Math))
    : k(jQuery, d, d.document, d.Math);
  ...
  d.fn.fullpage = function(c) {
    function y() {
      ...
      a.length &&
        (0 !== d('.fp-section.active').index('.fp-section') ||
          (0 === d('.fp-section.active').index('.fp-section') &&
            0 !== a.index())) &&
        U(a);
      la();
    ...
    function Ma() {
      ...
      d('.fp-section').each(function(a) {
        ...
        'undefined' !== typeof c.sectionsColor[a] &&
          b.css('background-color', c.sectionsColor[a]);
        'undefined' !== typeof c.anchors[a] &&
          b.attr('data-anchor', c.anchors[a]);
        'undefined' !== typeof c.anchors[a] &&
          b.hasClass('active') &&
          W(c.anchors[a], a);
        c.menu &&
          c.css3 &&
          d(c.menu).closest('.fullpage-wrapper').length &&
          d(c.menu).appendTo(q);
      ...
      c.navigation && Pa();
      c.scrollOverflow
        ? ('complete' === m.readyState && oa(), n.on('load', oa))
        : pa();
    }
    function Oa(a, b, g) {
      var f = 100 * g,
      ...
      1 < g && (c.controlArrows && Qa(a), c.slidesNavigation && Ra(a, g));
      b.each(function(a) {
        d(this).css('width', e + '%');
      ...
      a.length &&
      (0 !== d('.fp-section.active').index('.fp-section') ||
        (0 === d('.fp-section.active').index('.fp-section') && 0 !== a.index()))
        ? U(a)
    ...
    function Pa() {
      ...
      var a = d('#fp-nav');
      a.addClass(function() {
        return c.showActiveTooltip
          ? 'fp-show-active ' + c.navigationPosition
      ...
      d('#fp-nav')
        .find('li')
        .eq(d('.fp-section.active').index('.fp-section'))
        .find('a')
        .addClass('active');
    }
    function oa() {
    ...
    function pa() {
      ...
      c.scrollOverflowHandler.afterRender &&
        c.scrollOverflowHandler.afterRender(a);
      K(a);
      qa(a);
      d.isFunction(c.afterLoad) &&
        c.afterLoad.call(a, a.data('anchor'), a.index('.fp-section') + 1);
    ...
    function ra() {
      ...
      if (!c.autoScrolling || c.scrollBar) {
        for (
          var b = n.scrollTop(),
            g = 0,
            f = p.abs(b - m.querySelectorAll('.fp-section')[0].offsetTop),
        ...
        if (!a.hasClass('active') && !a.hasClass('fp-auto-height')) {
          ...
          v &&
            (a
              ...
              .siblings()
              .removeClass('active'),
            d.isFunction(c.onLeave) && c.onLeave.call(b, g, h, f),
            d.isFunction(c.afterLoad) && c.afterLoad.call(a, e, h),
            K(a),
            W(e, h - 1),
            c.anchors.length && ((z = e), Z(y, l, e, h)));
          clearTimeout(aa);
    ...
    function L(a, b) {
      if (l.m[a]) {
        var d, f;
        'down' == a
          ? ((d = 'bottom'), (f = e.moveSectionDown))
          : ((d = 'top'), (f = e.moveSectionUp));
        if (0 < b.length)
    ...
    function Sa(a) {
      ...
      if (!sa(a.target) && ca(b)) {
        ...
        v &&
          ...
          (F = b.y),
          (M = b.x),
          a.find('.fp-slides').length && p.abs(N - M) > p.abs(G - F)
            ? p.abs(N - M) > (n.width() / 100) * c.touchSensitivity &&
              (N > M
                ? l.m.right && e.moveSlideRight()
                : l.m.left && e.moveSlideLeft())
            : c.autoScrolling &&
              p.abs(G - F) > (n.height() / 100) * c.touchSensitivity &&
              (G > F ? L('down', g) : F > G && L('up', g)));
      }
    }
    function sa(a, b) {
      b = b || 0;
      ...
      return b < c.normalScrollElementTouchThreshold &&
        g.is(c.normalScrollElements)
        ? !0
        : b == c.normalScrollElementTouchThreshold
        ? !1
        : sa(g, ++b);
    }
    function ca(a) {
    ...
    function ua(a, b) {
      for (
        var c = 0, d = a.slice(p.max(a.length - b, 1)), e = 0;
        e < d.length;
        e++
      )
        c += d[e];
    ...
    function B(a) {
      ...
      if (c.autoScrolling && !O) {
        ...
        v &&
          ((b = ua(C, 10)),
          (g = ua(C, 70)),
          b >= g && e && (0 > f ? L('down', a) : L('up', a)));
        return !1;
      }
      c.fitToSection && x.stop();
    }
    function wa(a, b) {
      var g = ('undefined' === typeof b ? d('.fp-section.active') : b).find(
          '.fp-slides',
        ),
        ...
        if ('undefined' !== typeof f) {
          ...
          if (
            ...
            c.autoScrolling &&
              ...
              (f.isMovementUp
                ? d('.fp-section.active').before(
                    f.activeSection.nextAll('.fp-section'),
                  )
                : d('.fp-section.active').after(
                    f.activeSection
                      .prevAll('.fp-section')
                      .get()
                      .reverse(),
                  ),
              I(d('.fp-section.active').position().top),
              xa(),
              (f.wrapAroundElements = f.activeSection),
              (f.dest = f.element.position()),
              (f.dtop = f.dest.top),
              (f.yMovement = Y(f.element)));
    ...
    function Va(a) {
      if (c.css3 && c.autoScrolling && !c.scrollBar)
        ya('translate3d(0px, -' + a.dtop + 'px, 0px)', !0),
          c.scrollingSpeed
    ...
    function ea(a) {
      a.wrapAroundElements &&
        ...
        (a.isMovementUp
          ? d('.fp-section:first').before(a.wrapAroundElements)
          : d('.fp-section:last').after(a.wrapAroundElements),
        I(d('.fp-section.active').position().top),
        xa());
      a.element.find('.fp-scrollable').mouseover();
      d.isFunction(c.afterLoad) &&
        !a.localIsResizing &&
        c.afterLoad.call(a.element, a.anchorLink, a.sectionIndex + 1);
      qa(a.element);
      v = !0;
    ...
    function H(a, b) {
      ...
      if (c.onSlideLeave) {
        ...
        if (
          !n &&
          'none' !== r &&
          d.isFunction(c.onSlideLeave) &&
          !1 === c.onSlideLeave.call(y, k, h + 1, q, r, f)
        ) {
          w = !1;
          return;
        }
      ...
      !c.loopHorizontal &&
        ...
        e.find('.fp-controlArrow.fp-next').toggle(!b.is(':last-child')));
      e.hasClass('active') && Z(f, m, k, h);
      var t = function() {
        n ||
          (d.isFunction(c.afterSlideLoad) &&
            c.afterSlideLoad.call(b, k, h + 1, m, f));
        w = !1;
      };
      c.css3
        ? ((g = 'translate3d(-' + p.round(g.left) + 'px, 0px, 0px)'),
          Aa(a.find('.fp-slidesContainer'), 0 < c.scrollingSpeed).css(Ba(g)),
          (ha = setTimeout(
            function() {
              t();
            },
    ...
    function J(a) {
      ...
      e.length
        ? (h = b.scrollHeight(a))
        : ((h = a.get(0).scrollHeight),
          c.verticalCentered &&
            (h = a.find('.fp-tableCell').get(0).scrollHeight));
      f =
        r - parseInt(f.css('padding-bottom')) - parseInt(f.css('padding-top'));
      h > f
        ? e.length
          ? b.update(a, f)
          : (c.verticalCentered
              ? a.find('.fp-tableCell').wrapInner(d)
              : a.wrapInner(d),
            b.create(a, f))
        : b.remove(a);
      a.css('overflow', '');
    ...
    function Ea(a) {
      var b = d('.fp-section[data-anchor="' + a + '"]');
    ...
    function ma() {
      ...
      b.length && (a = a + '-' + f);
      a = a.replace('/', '-').replace('#', '');
      q[0].className = q[0].className.replace(
    ...
    e.setMouseWheelScrolling = function(a) {
      if (a) {
        ...
        m.addEventListener
          ? (m.removeEventListener('mousewheel', B, !1),
            m.removeEventListener('wheel', B, !1),
            m.removeEventListener('MozMousePixelScroll', B, !1))
          : m.detachEvent('onmousewheel', B);
    };
    ...
    e.reBuild = function(a) {
      if (!h.hasClass('fp-destroyed')) {
        ...
        (b = d('.fp-section.active').index('.fp-section')) &&
          e.silentMoveTo(b + 1);
        requestAnimFrame(function() {
          u = !1;
        });
    ...
    e.setResponsive = function(a) {
      ...
      a
        ? b ||
          ...
          d('#fp-nav').hide(),
          q.addClass('fp-responsive'))
        : b &&
          (e.setAutoScrolling(D.autoScrolling, 'internal'),
          e.setFitToSection(D.autoScrolling, 'internal'),
          d('#fp-nav').show(),
          q.removeClass('fp-responsive'));
    };
    var w = !1,
    ...
    c.normalScrollElements &&
      (t.on('mouseenter', c.normalScrollElements, function() {
    ...
    d('.fp-section').on('click touchstart', '.fp-controlArrow', function() {
      ...
      d(this).hasClass('fp-prev')
        ? l.m.left && e.moveSlideLeft(a)
        : l.m.right && e.moveSlideRight(a);
    });
    n.resize(Ca);
    var ia = r;
  ...
  ka = {
    ...
    scrollable: function(c) {
      return c.find('.fp-slides').length
        ? c.find('.fp-slide.active').find('.fp-scrollable')
