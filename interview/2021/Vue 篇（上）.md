...
由于文章字数限制，所以将 Vue 篇分为了上下两篇，剩余内容在下篇。
...
![pic_153daaa7.png](https://www.liangtengyu.com:9998/images/pic_153daaa7.png)

## 一、Vue 基础

### 1. Vue 的基本原理

当一个 Vue 实例创建时，Vue 会遍历 data 中的属性，用 Object.defineProperty（vue3.0 使用 proxy ）将它们转为 getter/setter，并且在内部追踪相关依赖，在属性被访问和修改时通知变化。 每个组件实例都有相应的 watcher 程序实例，它会在组件渲染的过程中把属性记录为依赖，之后当依赖项的 setter 被调用时，会通知 watcher 重新计算，从而致使它关联的组件得以更新。 ![pic_cb618a16.png](https://www.liangtengyu.com:9998/images/pic_cb618a16.png)
...
![pic_e8843630.png](https://www.liangtengyu.com:9998/images/pic_e8843630.png)
...
这种模式实现了 Model 和 View 的数据自动同步，因此开发者只需要专注于数据的维护操作即可，而不需要自己操作 DOM。 ![pic_2ea0d69f.png](https://www.liangtengyu.com:9998/images/pic_2ea0d69f.png)
...
在 Vue 中，对响应式处理利用的是 Object.defineProperty 对数据进行拦截，而这个方法并不能监听到数组内部变化，数组长度变化，数组的截取变化等，所以需要对这些操作进行 hack，让 Vue 能监听到其中的变化。 ![pic_5efd65a8.png](https://www.liangtengyu.com:9998/images/pic_5efd65a8.png) 那 Vue 是如何实现让这些数组方法实现元素的实时更新的呢，下面是 Vue 中对这些方法的封装：
...
简单来说就是，重写了数组中的那些原生方法，首先获取到这个数组的\_\_ob\_\_，也就是它的 Observer 对象，如果有新的值，就调用 observeArray 继续对新的值观察变化（也就是通过`target__proto__ == arrayMethods`来改变了数组实例的型），然后手动调用 notify，通知渲染 watcher，执行 update。
...
区别： ![pic_f77f5107.png](https://www.liangtengyu.com:9998/images/pic_f77f5107.png)
...
![pic_7ed1889c.png](https://www.liangtengyu.com:9998/images/pic_7ed1889c.png) （2）mergeOptions 的执行过程
...
`eventBus`事件总线适用于父子组件、非父子组件等之间的通信，使用步骤如下： （1）创建事件中心管理组件之间的通信
