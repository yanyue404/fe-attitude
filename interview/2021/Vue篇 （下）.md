...
由于单篇文章的字数限制，所以将 Vue 篇拆分为两部分，本篇为下篇。

注：2021.5.21 对之前发的面试题系列文章进行了更新，修改文中出现的部分错误。添加了思维导图，根据自己的面试经历以及牛客网等平台的面经，将面试题的考察频率进行大致的区分，可以针对性的进行复习。
...
![pic_61d45c3b.png](https://www.liangtengyu.com:9998/images/pic_61d45c3b.png)
...
简介： hash 模式是开发中默认的模式，它的 URL 带着一个\#，例如：[www.abc.com/\#/vue][www.abc.com_vue]，它的 hash 值就是`#/vue`。
...
简介： history 模式的 URL 中没有\#，它使用的是传统的路由分发模式，即用户在输入一个 URL 时，服务器会接收这个请求，并解析这个 URL，然后做出相应的逻辑处理。 特点： 当使用 history 模式时，URL 就像这样：[abc.com/user/id][abc.com_user_id]。相比 hash 模式更加好看。但是，history 模式需要后台配置支持。如果后台没有正确配置，访问时会返回 404。 API： history api 可以分为两大部分，切换历史状态和修改历史状态：
...
![pic_c0e452de.png](https://www.liangtengyu.com:9998/images/pic_c0e452de.png) Vuex 为 Vue Components 建立起了一个完整的生态圈，包括开发中的 API 调用一环。 （1）核心流程中的主要功能：
