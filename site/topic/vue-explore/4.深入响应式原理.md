...
2. get 收集时刻，vm 实例 在 \$mount 第一次初始化渲染页面的创建渲染 watcher 并执行其更新方法 **updateComponent** 初始化视图（render 函数进行依赖收集，在 render 函数中用到 data 属性值所在的 dep 实例中 push 到 vue 实例级别的 渲染 watcher， **updateComponent** 更新视图的方法）
