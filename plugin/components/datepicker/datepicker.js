(function() {
  var utils = {
    addClass: function(target, className) {
      class_arr = target.getAttribute('class')
  ...
  Calendar.prototype = {
    ...
    openPanel: function(target) {
      if (utils.hasClass(target, Calendar.originOpt.PICKERNAME)) {
        // 说明该元素已经挂载
        var only_key = utils.attr(target, Calendar.originOpt.PANELKEY);
        this.elem_wrap = document.querySelector(
          '.' + Calendar.originOpt.PANELSTR + only_key,
        );
    ...
    create: function(target) {
      ...
      div.className =
        Calendar.originOpt.PANELWRAPCLASS +
        ' ' +
        Calendar.originOpt.PANELSTR +
        only_key;
      div.innerHTML =
        this.getTemplate1() +
        this.getTbodyTemplate(this.dateOpt.year, this.dateOpt.month) +
        this.getTemplate2();
    ...
    getTemplate1: function() {
      var selectDate =
        this.dateOpt.year +
        '-' +
        utils.formatDate(this.dateOpt.month + 1) +
        '-' +
        utils.formatDate(this.dateOpt.date);
      return (
        '<div class="atie-calendar-mask"></div>' +
        ...
        '<input class="atie-calendar-input " placeholder="请选择日期" value="' +
        selectDate +
        '">' +
        '</div>' +
        '</div>' +
        ...
        '<a class="atie-calendar-year-select" role="button">' +
        this.dateOpt.year +
        '年</a>' +
        '<a class="atie-calendar-month-select" role="button">' +
        (this.dateOpt.month + 1) +
        '月</a>' +
        '</span>' +
    ...
    getTemplate2: function() {
      return (
        ...
        '<a class="atie-calendar-today-btn " role="button" title="' +
        this.dateOpt.curYear +
        '年' +
        (this.dateOpt.curMonth + 1) +
        '月' +
        this.dateOpt.curDate +
        '日">今天</a>' +
        '</span>' +
        '</div>' +
        '</div>' +
    ...
    getTbodyTemplate: function() {
      // 当月第一天日期对象
      var currentMonthFirstDateObj = new Date(
        this.dateOpt.year,
        this.dateOpt.month,
        1,
      );
      ...
      // 当月最后一天日期对象
      var currentMonthLastDateObj = new Date(
        this.dateOpt.year,
        this.dateOpt.month + 1,
        0,
      );
      ...
      // 上个月最后一天日期对象
      var lastMonthLastDateObj = new Date(
        this.dateOpt.year,
        this.dateOpt.month,
        0,
      );
      ...
      for (var i = 1; i <= 42; i++) {
        ...
        if (i < currentMonthFirstDay) {
          ...
          if (
            this.dateOpt.year === this.dateOpt.curYear &&
            this.dateOpt.month === this.dateOpt.curMonth &&
            this.dateOpt.curDate === date
          )
            className = 'atie-calendar-cell atie-calendar-today';
          if (
            this.dateOpt.selectYear === this.dateOpt.year &&
            this.dateOpt.selectMonth === this.dateOpt.month &&
            this.dateOpt.selectDate === date
          )
            className += ' atie-calendar-selected-date';
          if (this.dateOpt.date === date)
            className += ' atie-calendar-selected-day';
        }
        html +=
          '<td class="atie-calendar-cell ' +
          className +
          '"><div class="atie-calendar-date">' +
          date +
          '</div></td>';
    ...
    updateHtml: function(type) {
      ...
      switch (type) {
        case 'yearChange':
          this.elem_wrap.querySelector('.atie-calendar-year-select').innerHTML =
            this.dateOpt._year + '月';
          this.elem_wrap.querySelector(
            '.atie-calendar-month-select',
          ).innerHTML = this.dateOpt._month + 1 + '月';
          break;
        case 'monthChange':
          this.elem_wrap.querySelector(
            '.atie-calendar-month-select',
      ...
      if (this.isSelected) {
        ...
        this.elem_wrap.querySelector('.atie-calendar-input ').value =
          this.dateOpt._year +
          '-' +
          utils.formatDate(this.dateOpt._month + 1) +
          '-' +
          utils.formatDate(this.dateOpt._date);
        // callback
        Calendar.Opt.callBack &&
          Calendar.Opt.callBack(this.bindElem, {
