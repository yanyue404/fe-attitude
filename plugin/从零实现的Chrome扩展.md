...
class FilesPlugin {
  apply(compiler) {
    compiler.hooks.make.tap('FilePlugin', compilation => {
      ...
      !compilation.contextDependencies.has(resources) && compilation.contextDependencies.add(resources)
    })

    compiler.hooks.done.tapPromise('FilePlugin', () => {
      return Promise.all([exec('cp ./src/manifest.json ./dist/'), exec('cp -r ./public/static ./dist/static')])
    })
  }
}
...
export const App: FC = () => {
  ...
  const onSwitchChange = (
    type: typeof POPUP_CONTENT_ACTION.MENU | typeof POPUP_CONTENT_ACTION.KEYDOWN | typeof POPUP_CONTENT_ACTION.COPY,
    checked: boolean,
    once = false
  ) => {
  ...
  return <div className={cs(style.container)}>{/* xxx */}</div>
}
```
