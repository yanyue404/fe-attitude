...
/**
 * @param String
 *            id 对话框的id，若不传递，则默认为zhihuiyun_box
 */
PWMENU_ZINDEX=1001;
...
function getPWBox(type){
	if (getObj(type||'zhihuiyun_box')) {
		return getObj(type||'zhihuiyun_box');
	}
	var zhihuiyun_box	= elementBind('div',type||'zhihuiyun_box','','position:absolute;left:-10000px');

	document.body.appendChild(zhihuiyun_box);
	return zhihuiyun_box;
}

function getPWContainer(id,border){
	id = id || '';
	if (getObj(id||'zhihuiyun_box')) {
		var zhihuiyun_box = getObj(id||'zhihuiyun_box');
	} else {
		var zhihuiyun_box = getPWBox(id);
	}
	...
	if (border == 1) {
		zhihuiyun_box.innerHTML = '<div class="popout"><div id="'+id+'box_container"></div></div>';
	} else {
		zhihuiyun_box.innerHTML = '<div class="popout"><table border="0" cellspacing="0" cellpadding="0"><tbody><tr><td class="bgcorner1"></td><td class="pobg1"></td><td class="bgcorner2"></td></tr><tr><td class="pobg4"></td><td><div class="popoutContent" id="'+id+'box_container"></div></td><td class="pobg2"></td></tr><tr><td class="bgcorner4"></td><td class="pobg3"></td><td class="bgcorner3"></td></tr></tbody></table></div>';
	}
...
(function(win,doc) {
    ...
    Dialog.prototype = {
        ...
        show:function(options) {
                ...
                createButton = function(){// 创建按钮
                    ...
                    if(btn){
                        for(var i = 0,j = btn.length;i < j;i++ ) {
                            html.push('<span class="bt2"><span><button class="zhydb_dialoag_button" type="button">'+ btn[i][0] +'</button></span></span>');
                        }
                    }else {
                        if(opt.type === 'confirm') {
                            html.push('<span class="btn2"><span><button type="button" class="zhydb_dialoag_ok">'+ opt.okText +'</button></span></span>');
                        }
                        html.push('<span class="bt2"><span><button type="button" class="zhydb_dialoag_close">'+ opt.closeText +'</button></span></span>');
                    }
                    return html.join('');
                }
                // timeout;
            if(!opt.showObj) {
				...
				if(opt.button) {
				    var customBtn = getElementsByClassName('zhydb_dialoag_button',box),buttons = opt.button;
				    if(customBtn.length){
		            ...
		            var closeBtn = getElementsByClassName('zhydb_dialoag_close',box),
                        okBtn = getElementsByClassName('zhydb_dialoag_ok',box);
                   if(closeBtn.length){
                ...
                var msgObj = getElementsByClassName('message',obj),
                    ...
                    msgConfirm = getElementsByClassName('confirm',obj);
                ...
                if( msgConfirm.length ) { msgConfirm[0].onclick = function() {self.options.onOk && self.options.onOk(); self.close();}}
                //初始化配置
                obj.style.left= defaultCfg.left;
                obj.style.top= defaultCfg.top;
            }
	...
	win.messageTip=function(st){
			if(!getObj("zhydb_all_tip")){
				return false;
			}
			this.dd = document.documentElement;
			this.db = document.body;
			this.scrollTop=null;//顶部距离
			this.ele=getObj("zhydb_all_tip");
			this.closeBtn=getObj("zhydb_all_tclose");
			this.st=st||90;
	...
	messageTip.prototype={
		...
		close:function(){
			var self = this,url="";
			if(typeof zhydb_baseurl!="undefined"){
				url=zhydb_baseurl+"/";
			}
			var num=this.closeBtn.getAttribute("data-num");
			ajax.send(url+'zhydb_ajax.php?action=clearmessage&num='+num,'',function(){
	...
	usercard.prototype = {
		...
		"fillCont":function(ele,json){
				...
				var attentionHtml=[];
				attentionHtml = mine ? '<a href="javascript:void(0)" onclick="'+(attention==1?'':'Attention.add(this,'+uid+',1)')+'" class="'+(attention==1?'add_following':'s4 add_follow')+' mr10">'+(attention==1?'已关注':'加关注')+'</a><span class="gray2 mr10">|</span><a href="javascript:void(0)" onclick="sendmsg(\'zhydb_ajax.php?action=msg&touid='+uid+'\', \'\', this)" class="s4">发消息</a>' : '';
				//勋章html
	...
	/*新功能气泡
	*new Bubble(a).init();
	*a:页面中的气泡元素ID  如: [{"name":"zhydb_all_tip_kmd","id":"markoperation"},{"name":"zhydb_all_tip_medal","id":"applicationcenter"}]
	*/
	win.Bubble=function(ids){
		this.ids=ids;
		this.info={};//程序输出的气泡ID，表示不显示此气泡
		this.uid=(typeof zhihuiyunuid!="undefined")?zhihuiyunuid:null;
	}
	Bubble.prototype={
		init:function(){
			if(this.uid==null){
		...
		hide:function(obj,str,ele){
			ajax.send("zhydb_ajax.php?action=bubble&uid="+this.uid+"&sign="+str,"",function(){
