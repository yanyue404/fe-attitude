...
<div class="p10"><a href="javascript:closep();" class="adel">关闭</a>提示</div><div class="popBottom"><span class="btn2"><span><button type="button">点击这里复制代码</button></span></span></div></div></td><td class="pobg2"></td></tr><tr><td class="bgcorner4"></td><td class="pobg3"></td><td class="bgcorner3"></td></tr></tbody></table></div>';
			...
			var btn = getObj('zhihuiyun_box').getElementsByTagName('button')[0];
			clip.glue(btn);
...
userCard = {
	...
	show : function(id) {
		...
		if (typeof userCard.data[sx[3]] == 'undefined') {
			try {
				ajax.send(getObj('headbase').href + 'zhydb_ajax.php?action=showcard&uid=' + sx[3]+ '&rnd='+Math.random(), '', function() {
...
function openjobpop(param){
	var param = param || '';
	ajax.send('zhydb_ajax.php?action=jobpop',param,function(){
...
var Attention = {
	add : function(obj, touid, recommend) {
		ajax.send('zhydb_ajax.php?action=addattention&touid=' + touid + (recommend ? '&recommend=' + recommend : ''), '', function() {
	...
	del : function(touid) {
		ajax.send('zhydb_ajax.php?action=delattention&touid=' + touid, '', function() {
			var rText = ajax.request.responseText.split('\t');
			if (rText[0] == 'success') {
				window.location.reload();
			} else {
