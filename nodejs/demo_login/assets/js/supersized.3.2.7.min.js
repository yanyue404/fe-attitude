...
(function(a) {
  ...
  a.supersized = function(b) {
    ...
    d._build = function() {
      ...
      while (g <= d.options.slides.length - 1) {
        ...
        if (g == d.options.start_slide - 1) {
          ...
          if (d.options.thumb_links) {
            d.options.slides[g].thumb
              ? (i = d.options.slides[g].thumb)
      ...
      if (d.options.thumb_links && vars.thumb_tray.length) {
        a(vars.thumb_tray).append(
          '<ul id="' + vars.thumb_list.replace('#', '') + '">' + f + '</ul>',
        );
      }
      a(d.el).append(e);
      if (d.options.thumbnail_navigation) {
        vars.current_slide - 1 < 0
          ? (prevThumb = d.options.slides.length - 1)
          : (prevThumb = vars.current_slide - 1);
        a(vars.prev_thumb)
          .show()
          .html(a('<img/>').attr('src', d.options.slides[prevThumb].image));
        vars.current_slide == d.options.slides.length - 1
          ? (nextThumb = 0)
          : (nextThumb = vars.current_slide + 1);
        a(vars.next_thumb)
          .show()
          .html(a('<img/>').attr('src', d.options.slides[nextThumb].image));
      }
      d._start();
    };
    d._start = function() {
      if (d.options.start_slide) {
        ...
        vars.current_slide = Math.floor(
          Math.random() * d.options.slides.length,
        );
      }
      ...
      if (d.options.random) {
        ...
        for (
          var h, m, k = arr.length;
          k;
          h = parseInt(Math.random() * k),
            m = arr[--k],
            arr[k] = arr[h],
            arr[h] = m
        ) {}
      ...
      if (d.options.slides.length > 1) {
        if (d.options.slides.length > 2) {
          vars.current_slide - 1 < 0
            ? (loadPrev = d.options.slides.length - 1)
            : (loadPrev = vars.current_slide - 1);
          var g = d.options.slides[loadPrev].url
            ? "href='" + d.options.slides[loadPrev].url + "'"
            : '';
          ...
          q.load(function() {
            a(this)
              .data('origWidth', a(this).width())
              .data('origHeight', a(this).height());
            d.resizeNow();
          });
        }
      } else {
      ...
      if (d.options.slides.length > 1) {
        ...
        g = d.options.slides[loadNext].url
          ? "href='" + d.options.slides[loadNext].url + "'"
          : '';
    ...
    d.nextSlide = function() {
      ...
      vars.current_slide + 1 == d.options.slides.length
      ...
      if (!a(k).html()) {
        ...
        imageLink = d.options.slides[loadSlide].url
          ? "href='" + d.options.slides[loadSlide].url + "'"
          : '';
        var f = a('<img src="' + d.options.slides[loadSlide].image + '"/>');
        f.appendTo(k)
          .wrap('<a ' + imageLink + j + '></a>')
          .parent()
          .parent()
          .addClass('image-loading')
      ...
      if (d.options.thumbnail_navigation == 1) {
        ...
        a(vars.prev_thumb).html(
          a('<img/>').attr('src', d.options.slides[prevThumb].image),
        );
        nextThumb = loadSlide;
        a(vars.next_thumb).html(
          a('<img/>').attr('src', d.options.slides[nextThumb].image),
        );
      }
      if (
        typeof theme != 'undefined' &&
      ...
      switch (d.options.transition) {
        ...
        case 'carouselRight':
          ...
          e.animate(
            { left: -d.$el.width(), avoidTransforms: false },
            d.options.transition_speed,
          );
          break;
        case 7:
    ...
    d._placeSlide = function(e) {
      ...
      if (e == 'next') {
        ...
        if (e == 'prev') {
          vars.current_slide - 1 < 0
            ? (loadSlide = d.options.slides.length - 1)
            : (loadSlide = vars.current_slide - 1);
          var g = d.el + ' li:eq(' + loadSlide + ')';
          if (!a(g).html()) {
            var h = d.options.new_window ? ' target="_blank"' : '';
    ...
    d.afterAnimation = function() {
      ...
      if (vars.update_images) {
        vars.current_slide - 1 < 0
          ? (setPrev = d.options.slides.length - 1)
